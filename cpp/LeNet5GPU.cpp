...
// CUDA kernel for forward pass of convolution layer
__global__ void convolutionForward(const double* input, const double* weights, const double* biases,
                                    double* output, int in_channels, int out_channels,
                                    int input_height, int input_width, int kernel_size,
                                    int stride, int padding) {
    // Calculate global thread index
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int out_size = (input_height - kernel_size + 2 * padding) / stride + 1;

    if (index < out_channels * out_size * out_size) {
        int o = index / (out_size * out_size);
        int yx = index % (out_size * out_size);
        int y = yx / out_size;
        int x = yx % out_size;
            ...
            for (int ky = 0; ky < kernel_size; ++ky) {
                for (int kx = 0; kx < kernel_size; ++kx) {
                    ...
                    if (in_y >= 0 && in_y < input_height && in_x >= 0 && in_x < input_width) {
                        ...
                        int weight_index = ((o * in_channels + c) * kernel_size + ky) * kernel_size + kx;
...
// CUDA kernel for forward pass of fully connected layer
__global__ void fullyConnectedForward(const double* input, const double* weights, const double* biases,
                                        double* output, int in_features, int out_features) {
    // Calculate global thread index
    int index = blockIdx.x * blockDim.x + threadIdx.x;

    if (index < out_features) {
        double sum = biases[index];
        for (int i = 0; i < in_features; ++i) {
            sum += input[i] * weights[index * in_features + i];
        }
        output[index] = sum;
    }
}
...
int main() {
    ...
    cudaMalloc(&d_weights, weightsSize * sizeof(double));
    cudaMalloc(&d_biases, biasesSize * sizeof(double));
    cudaMalloc(&d_output, outputSize * sizeof(double));

    // Copy data to GPU
    cudaMemcpy(d_input, input.data(), inputSize * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(d_weights, weights.data(), weightsSize * sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(d_biases, biases.data(), biasesSize * sizeof(double), cudaMemcpyHostToDevice);

    // Launch CUDA kernel for convolution forward pass
    convolutionForward<<<(outputSize + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK, THREADS_PER_BLOCK>>>(
        d_input, d_weights, d_biases, d_output, in_channels, out_channels,
